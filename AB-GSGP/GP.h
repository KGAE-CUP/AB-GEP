 ...
 *
 */
class symbol{ //new symbol(1,0,0,"(",-1)
	public:
...
void create_population_copy(population **p, int i,int a,int b);
...
void create_grow_tree_copy(node **el, int depth, node *parent, int max_depth,int a,int b,int flag);
...
* \file             GP.h
*/
void evaluate_unseen_new_data(population **p, ofstream& OUT);
...
void read_config_file(cfg *config){
    ...
    f.close();
    if(config->p_crossover<0 || config->p_mutation<0 || (config->p_crossover+config->p_mutation)>2){
    	cout<<config->p_crossover;
		cout<<config->p_mutation;
...
//		min_depth=0;
	...
	while(j>=min_depth){ //初始的时候min_depth=1 
...
//		min_depth=0;
	...
	while(j>=min_depth){ //初始的时候min_depth=1 
		if(j<config.max_depth_creation){ //max_depth_creation=5
			for(int k=0; k<(int)(ceil((double)sub_pop/2)); k++){
				(*p)->individuals[i]=new node;
				create_full_tree_copy((node**)&((*p)->individuals[i]),0, NULL, j,a,b,1);
				i++;
				(*p)->num_ind++;
			}
			for(int k=0; k<(int)(floor((double)sub_pop/2)); k++){
				(*p)->individuals[i]=new node;
				create_grow_tree_copy((node**)&((*p)->individuals[i]),0, NULL ,j,a,b,1);
				i++;
				(*p)->num_ind++;					
			}
		}
	...
	for(int k=0; k<config.random_tree; k++){
				(*p)->individuals[i]=new node;
				create_grow_tree_copy((node**)&((*p)->individuals[i]),0, NULL, config.max_depth_creation,a,b,1);
				i++;
				(*p)->num_ind++;
    }
}
...
void create_grow_tree_copy(node **el, int depth, node *parent, int max_depth,int a,int b,int flag){
	...
	if(flag<=2)
	{
		...
		for (int i=0; i<(*el)->root->arity; i++){
			...
			{
			  (*el)->children[i]=new node;
			  create_grow_tree_copy(((node **)&((*el)->children[i])), depth+1, *el, max_depth,a,b,flag+1);	
			}else{
			  (*el)->children[i]=new node;
			  create_grow_tree_copy(((node **)&((*el)->children[i])), depth+1, *el, max_depth,a,b,flag+2);
			}
...
//参数index用来指定是哪个符号，也就是id；flag用来跳出循环 
void create_full_tree_copy(node **el, int depth, node *parent, int max_depth,int a,int b, int flag){
	int index=0;
	if(flag==1)
	{
	...
	if(flag<=2)//只取前三位 
	{
		...
		for (int i=0; i<(*el)->root->arity; i++){
			...
			{
			  (*el)->children[i]=new node;
			  create_full_tree_copy(((node **)&((*el)->children[i])), depth+1, *el, max_depth,a,b,flag+1);	
			}else{
				(*el)->children[i]=new node;
			  create_full_tree_copy(((node **)&((*el)->children[i])), depth+1, *el, max_depth,a,b,flag+2);
			}
