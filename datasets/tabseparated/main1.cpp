...
int my_run(int argc, const char **argv,int k){
   ...
   double historybest_train=numeric_limits<double>::max();
     double historybest_test=numeric_limits<double>::max();

    // name of the file with training instances 
    char path_in[50]="/data1/users/Teachers/luq/GSGP/24train.txt";
    // name of the file with test instances
    char path_test[50]="/data1/users/Teachers/luq/GSGP/24test.txt";
	...
	// if USE_TEST_SET is different from 1 the system will perform the usual evolutionary process
		...
		*/
		ofstream fitness_train("/data1/users/Teachers/luq/GSGP/oldtrain/shiyingdu/24."+std::to_string(k)+".txt",ios::out);
		/*
		pointer to the file fitnesstest.txt containing the training fitness of the best individual at each generation
		*/
		ofstream fitness_test("/data1/users/Teachers/luq/GSGP/oldtest/shiyingdu/24."+std::to_string(k)+".txt",ios::out);
   
    ofstream zhongqun_train("/data1/users/Teachers/luq/GSGP/oldtrain/zhongqun/24."+std::to_string(k)+".txt",ios::out);
    ofstream zhongqun_test("/data1/users/Teachers/luq/GSGP/oldtest/zhongqun/24."+std::to_string(k)+".txt",ios::out);
